...
Ri=48; %May need to be divisible by dx,dy,dz
...
while coverror(h)>0.05;
   tic
    ...
    x_i(1,t_count) = x_1+Pos_Std*randn(1);
    x_i(2,t_count) = x_2+Pos_Std*randn(1);
    x_i(3,t_count) = x_3+Pos_Std*randn(1);
    y_i(1,t_count) = y_1+Pos_Std*randn(1);
    y_i(2,t_count) = y_2+Pos_Std*randn(1);
    y_i(3,t_count) = y_3+Pos_Std*randn(1);
    z_i(1,t_count) = z_1+Pos_Std*randn(1);
    z_i(2,t_count) = z_2+Pos_Std*randn(1);
    z_i(3,t_count) = z_3+Pos_Std*randn(1);
...
if  t_count>101
if mean(e_hat_save(kk,t_count-100:t_count))<Ep_star && Global_Flag(kk)==0 
