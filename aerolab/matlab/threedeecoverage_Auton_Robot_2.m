...
K_u=5e-8; K_v=5e-8; K_w=5e-8; K_r=1e-7; K_s=1e-7;
%5e-8 and 1e-9 was used for robos submission trial one
r_i=25;
...
%New on 5/19/16
%Old Value for Trial One was 5e8
Ep_star=1e6; %Code Test Value
...
subdomain_flag=zeros(k,1);
Scan_Flag=zeros(k,1);
Scan_Transfer_Flag=zeros(k,1);
Scan_Spin_Flag=zeros(k,1);
Scan_pts_CTR=zeros(k,1);


%set up coordinates
dx=5; dy=5; dz=5; droll=.5; dpitch=.5; dyaw=.5;
%set(gcf,'visible','off')
dt=.1;
time=0:dt:50000;
...
quad_list = {'/vicon/hbirdw/hbirdw' '/vicon/hbirdy/hbirdy' '/vicon/hbirddg/hbirddg' '/vicon/hbirdg/hbirdg' '/vicon/hbirdYE/hbirdYE' '/vicon/hbirdgr/hbirdgr'};
...
Ri=50; %May need to be divisible by dx,dy,dz
...
Mode_Save=zeros(k,length(time));
act_accum_1=zeros(1,length(time));
act_accum_2=zeros(1,length(time));
act_accum_3=zeros(1,length(time));
time_save=zeros(1,length(time));
...
while (1);
    ...
    [Volt_sort,k_v]=sort([max(volt_1,V_cr),max(volt_2,V_cr),max(volt_3,V_cr)]);
    Volt_save(t_count,:)=[volt_1,volt_2,volt_3];
    R_cr=min(((1/3)*((length(z_coord)*dz)-dz)^3)^(1/3),kappa*(Volt_sort(1)-V_cr));
    R_1=(((length(z_coord)*dz)-dz)^3-((((length(z_coord)*dz)-dz)^3-R_cr^3)/(k-1)))^(1/3);
    R_2=((R_1)^3-((((length(z_coord)*dz)-dz)^3-R_cr^3)/(k-1)))^(1/3); 
    ...
    if z_i(kk,t_count)<min(z_coord)
        ...
        x_left(kk)=max(1,curr_x_Indx_Num(kk)-1*(Ri+2*r_i)/dx);
        x_right(kk)=min(length(x_coord),curr_x_Indx_Num(kk)+1*(Ri+2*r_i)/dx);
        y_left(kk)=max(1,curr_y_Indx_Num(kk)-1*(Ri+2*r_i)/dy);
        y_right(kk)=min(length(y_coord),curr_y_Indx_Num(kk)+1*(Ri+2*r_i)/dy);
        z_left(kk)=max(1,curr_z_Indx_Num(kk)-1*(Ri+2*r_i)/dz);
        z_right(kk)=min(length(z_coord),curr_z_Indx_Num(kk)+1*(Ri+2*r_i)/dz);
    end
    Q_mod1=Q;
    Q_mod2=Q;
    Q_mod3=Q;
    
     %Set Hemispherical Boundaries
    if k_v(1)==1
        Q_mod1(:,:,max(1,int64(R_cr*(1/dz))):length(z_coord))=c_star;
        Q_mod1(int64((1/2)*length(x_coord))+int64(R_cr*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod1(1:int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)),:,:)=c_star;
        Q_mod1(:,int64((1/2)*length(y_coord))+int64(R_cr*(1/dy)):length(y_coord),:)=c_star;
        Q_mod1(:,1:int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)),:)=c_star;
        
    end 
    if k_v(1)==2
        Q_mod2(:,:,max(1,int64(R_cr*(1/dz))):length(z_coord))=c_star;
        Q_mod2(int64((1/2)*length(x_coord))+int64(R_cr*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod2(1:int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)),:,:)=c_star;
        Q_mod2(:,int64((1/2)*length(y_coord))+int64(R_cr*(1/dy)):length(y_coord),:)=c_star;
        Q_mod2(:,1:int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)),:)=c_star;
    end
    if k_v(1)==3
        Q_mod3(:,:,max(1,int64(R_cr*(1/dz))):length(z_coord))=c_star;
        Q_mod3(int64((1/2)*length(x_coord))+int64(R_cr*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod3(1:int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)),:,:)=c_star;
        Q_mod3(:,int64((1/2)*length(y_coord))+int64(R_cr*(1/dy)):length(y_coord),:)=c_star;
        Q_mod3(:,1:int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)),:)=c_star;  
            
    end
    if k_v(2)==1
        Q_mod1(int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)):int64((1/2)*length(x_coord))+int64(R_cr*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)):int64((1/2)*length(y_coord))+int64(R_cr*(1/dy))...
            ,1:int64(R_cr*(1/dz)))=c_star;
        Q_mod1(int64((1/2)*length(x_coord))+int64(R_1*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod1(1:int64((1/2)*length(x_coord))-int64(R_1*(1/dx)),:,:)=c_star;
        Q_mod1(:,int64((1/2)*length(y_coord))+int64(R_1*(1/dy)):length(y_coord),:)=c_star;
        Q_mod1(:,1:int64((1/2)*length(y_coord))-int64(R_1*(1/dy)),:)=c_star;
    end
    if k_v(2)==2
        Q_mod2(int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)):int64((1/2)*length(x_coord))+int64(R_cr*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)):int64((1/2)*length(y_coord))+int64(R_cr*(1/dy))...
            ,1:int64(R_cr*(1/dz)))=c_star;
        Q_mod2(int64((1/2)*length(x_coord))+int64(R_1*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod2(1:int64((1/2)*length(x_coord))-int64(R_1*(1/dx)),:,:)=c_star;
        Q_mod2(:,int64((1/2)*length(y_coord))+int64(R_1*(1/dy)):length(y_coord),:)=c_star;
        Q_mod2(:,1:int64((1/2)*length(y_coord))-int64(R_1*(1/dy)),:)=c_star;
    end
    if k_v(2)==3
        Q_mod3(int64((1/2)*length(x_coord))-int64(R_cr*(1/dx)):int64((1/2)*length(x_coord))+int64(R_cr*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_cr*(1/dy)):int64((1/2)*length(y_coord))+int64(R_cr*(1/dy))...
            ,1:int64(R_cr*(1/dz)))=c_star;
        Q_mod3(int64((1/2)*length(x_coord))+int64(R_1*(1/dx)):length(x_coord),:,:)=c_star;
        Q_mod3(1:int64((1/2)*length(x_coord))-int64(R_1*(1/dx)),:,:)=c_star;
        Q_mod3(:,int64((1/2)*length(y_coord))+int64(R_1*(1/dy)):length(y_coord),:)=c_star;
        Q_mod3(:,1:int64((1/2)*length(y_coord))-int64(R_1*(1/dy)),:)=c_star;
    end
    if k_v(3)==1 
        Q_mod1(int64((1/2)*length(x_coord))-int64(R_1*(1/dx)):int64((1/2)*length(x_coord))+int64(R_1*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_1*(1/dy)):int64((1/2)*length(y_coord))+int64(R_1*(1/dy))...
            ,1:int64(R_1*(1/dz)))=c_star;
    end
    if k_v(3)==2
        Q_mod2(int64((1/2)*length(x_coord))-int64(R_1*(1/dx)):int64((1/2)*length(x_coord))+int64(R_1*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_1*(1/dy)):int64((1/2)*length(y_coord))+int64(R_1*(1/dy))...
            ,1:int64(R_1*(1/dz)))=c_star;
    end
    if k_v(3)==3
         Q_mod3(int64((1/2)*length(x_coord))-int64(R_1*(1/dx)):int64((1/2)*length(x_coord))+int64(R_1*(1/dx))...
            ,int64((1/2)*length(y_coord))-int64(R_1*(1/dy)):int64((1/2)*length(y_coord))+int64(R_1*(1/dy))...
            ,1:int64(R_1*(1/dz)))=c_star;
    end   
   ...
   for jj=1:k
       if jj==1;
            if Global_Flag(jj)==0 && Scan_Flag(jj)==0;
           [command_1,Si_1(:,:,:,jj)]=Local_Cov2(Q_mod1(x_left(1):x_right(1),y_left(1):y_right(1),z_left(1):z_right(1)),[x_i(jj,t_count),y_i(jj,t_count),z_i(jj,t_count),pitch_i(jj,t_count),yaw_i(jj,t_count),roll_i(jj,t_count)],x_coord,y_coord,z_coord,dx,dy,dz,K_u,K_v,K_w,K_r,K_s,Ri,alpha,c_star,r_i);
            ...
            if Global_Flag(jj)==1 || (Scan_Flag(jj)==1)
...
%Vicon Drop Safety Check
if t_count>10
if x_i(kk,t_count)==x_i(kk,t_count-1) && y_i(kk,t_count)==y_i(kk,t_count-1) && z_i(kk,t_count)==z_i(kk,t_count-1)
           ui(kk)=0;
       vi(kk)=0;
       wi(kk)=0;
       fprintf('!!COMMAND ZERO FROM VICON!!!!\n')
end
end
...
if Global_Flag(kk)==1 && sqrt((x_i(kk,t_count+1)-x_dest(kk))^2+(y_i(kk,t_count+1)-y_dest(kk))^2+(z_i(kk,t_count+1)-z_dest(kk))^2)<cap_tol && orient_norm(kk)<orient_tol;
    ...
    if mean(e_hat_save(kk,t_count-100:t_count-1))<Ep_star && sum(Scan_Flag)==0
        Scan_Flag(kk)=1;
        Scan_pts=[x_scan,y_scan,z_scan,
                  x_scan,y_scan,min(max(z_coord),z_scan+(sqrt(2)/4)*Ri);
                  x_scan,y_scan,min(max(z_coord),z_scan+(sqrt(2)/2)*Ri);
                  max(min(x_coord),x_scan-(sqrt(2)/8)*Ri*cot(alpha)),y_scan,min(max(z_coord),z_scan+(sqrt(2)/4)*Ri+(sqrt(2)/8)*Ri);
                  min(max(x_coord),x_scan+(sqrt(2)/8)*Ri*cot(alpha)),y_scan,min(max(z_coord),z_scan+(sqrt(2)/4)*Ri+(sqrt(2)/8)*Ri);
                  min(max(x_coord),x_scan+(sqrt(2)/8)*Ri*cot(alpha)),y_scan,min(max(z_coord),z_scan+(sqrt(2)/8)*Ri);
                  max(min(x_coord),x_scan-(sqrt(2)/8)*Ri*cot(alpha)),y_scan,min(max(z_coord),z_scan+(sqrt(2)/8)*Ri);
                  max(min(x_coord),x_scan-(sqrt(2)/8)*Ri*cot(alpha)),y_scan,max(min(z_coord),z_scan-(sqrt(2)/8)*Ri);
                  min(max(x_coord),x_scan+(sqrt(2)/8)*Ri*cot(alpha)),y_scan,max(min(z_coord),z_scan-(sqrt(2)/8)*Ri);
                  min(max(x_coord),x_scan+(sqrt(2)/8)*Ri*cot(alpha)),y_scan,max(min(z_coord),z_scan-(sqrt(2)/4)*Ri-(sqrt(2)/8)*Ri);
                  max(min(x_coord),x_scan-(sqrt(2)/8)*Ri*cot(alpha)),y_scan,max(min(z_coord),z_scan-(sqrt(2)/4)*Ri-(sqrt(2)/8)*Ri);
                  x_scan,y_scan,max(min(z_coord),z_scan-(sqrt(2)/2)*Ri);
                  x_scan,y_scan,max(min(z_coord),z_scan-(sqrt(2)/4)*Ri)];
        Scan_Transfer_Flag(kk)=1;
        Scan_Spin_Flag(kk)=0;
        Scan_pts_CTR(kk)=1;
        x_dest(kk)=Scan_pts(Scan_pts_CTR(kk),1);
        y_dest(kk)=Scan_pts(Scan_pts_CTR(kk),2);
        z_dest(kk)=Scan_pts(Scan_pts_CTR(kk),3);
        pitch_dest(kk)=0;
        yaw_dest(kk)=yaw_i(kk,t_count);
                  
    end
end

if Scan_Flag(kk)==1 && Scan_Transfer_Flag(kk)==1 && sqrt((x_i(kk,t_count+1)-x_dest(kk))^2+(y_i(kk,t_count+1)-y_dest(kk))^2+(z_i(kk,t_count+1)-z_dest(kk))^2)<cap_tol && orient_norm(kk)<orient_tol;
Scan_Transfer_Flag(kk)=0;
Scan_Spin_Flag(kk)=1;
yaw_dest(kk)=yaw_dest(kk)-dyaw;
end
if Scan_Flag(kk)==1 && Scan_Spin_Flag(kk)==1 && sqrt((x_i(kk,t_count+1)-x_dest(kk))^2+(y_i(kk,t_count+1)-y_dest(kk))^2+(z_i(kk,t_count+1)-z_dest(kk))^2)<cap_tol && orient_norm(kk)<orient_tol;
if Scan_pts_CTR(kk)<13
Scan_Transfer_Flag(kk)=1;
Scan_Spin_Flag(kk)=0;
Scan_pts_CTR(kk)=Scan_pts_CTR(kk)+1;
x_dest(kk)=Scan_pts(Scan_pts_CTR(kk),1);
y_dest(kk)=Scan_pts(Scan_pts_CTR(kk),2);
z_dest(kk)=Scan_pts(Scan_pts_CTR(kk),3);
pitch_dest(kk)=0;
...
if h(kk)==1
    coverroRinit=trapz(trapz(trapz(max(0,c_star-Q(:,:,:)).^3)))*dx*dy*dz;
end
 
coverror(h(kk))=trapz(trapz(trapz(max(0,c_star-Q(:,:,:)).^3)))*dx*dy*dz;
coverror(h(kk))=coverror(h(kk))/coverroRinit;
...
if  t_count>101
if mean(e_hat_save(kk,t_count-100:t_count-1))<Ep_star && Global_Flag(kk)==0 && Scan_Flag(kk)==0
...
%Go ahead and sk subdomain transfer mode code here
if kk==k_v(1) %if you're the crial agent
if abs(x_i(kk,t_count+1)-(1/2)*max(x_coord))>R_cr || abs(y_i(kk,t_count+1)-(1/2)*max(y_coord))>R_cr || abs(z_i(kk,t_count+1)-min(z_coord))>R_cr
subdomain_flag(kk)=1;
Global_Flag(kk)=1;
Scan_Flag(kk)=0;
x_dest(kk)=(1/2)*max(x_coord);
y_dest(kk)=(1/2)*max(y_coord);
z_dest(kk)=min(z_coord);
yaw_dest(kk)=yaw_i(kk,t_count+1);
pitch_dest(kk)=pitch_i(kk,t_count+1);
end
if subdomain_flag(kk)==1 && abs(x_i(kk,t_count+1)-(1/2)*max(x_coord))<R_cr && abs(y_i(kk,t_count+1)-(1/2)*max(y_coord))<R_cr && abs(z_i(kk,t_count+1)-min(z_coord))<R_cr
    subdomain_flag(kk)=0;
    Global_Flag(kk)=0;
end
end

if kk==k_v(2) %if you're the intermediate agent
if abs(x_i(kk,t_count+1)-(1/2)*max(x_coord))>R_1 || abs(y_i(kk,t_count+1)-(1/2)*max(y_coord))>R_1 || abs(z_i(kk,t_count+1)-min(z_coord))>R_1
subdomain_flag(kk)=1;
Global_Flag(kk)=1;
Scan_Flag(kk)=0;
x_dest(kk)=(1/2)*max(x_coord);
...
if subdomain_flag(kk)==1 && abs((x_i(kk,t_count+1)-(1/2)*max(x_coord)))<R_1 && abs((y_i(kk,t_count+1)-(1/2)*max(y_coord)))<R_1 && abs((z_i(kk,t_count+1)-min(z_coord)))<R_1
    subdomain_flag(kk)=0;
    Global_Flag(kk)=0;
end
end

if kk==k_v(3) && subdomain_flag(kk)==1 %if you are unconstrained and subdomain is on, turn it off
subdomain_flag(kk)=0;
Global_Flag(kk)=0;
...
if Global_Flag(kk)==0 && Scan_Flag(kk)==0
Mode_Save(kk,t_count)=0; %Local Mode
end
if Global_Flag(kk)==1 && subdomain_flag(kk)==0
    Mode_Save(kk,t_count)=1; %Global Mode
end
if Global_Flag(kk)==1 && subdomain_flag(kk)==1
    Mode_Save(kk,t_count)=2; % Subdomain Mode
end
if Scan_Flag(kk)==1
    Mode_Save(kk,t_count)=3; % Scan Mode
end
end
...
act_accum_2(t_count)=abs(ui(2))+abs(vi(2))+abs(wi(2))+abs(ri(2))+abs(si(2));
act_accum_3(t_count)=abs(ui(3))+abs(vi(3))+abs(wi(3))+abs(ri(3))+abs(si(3));
...
if Global_Flag(3)==1
Dest_Curr3=[x_i(3,t_count),x_dest(3),y_i(3,t_count),y_dest(3),z_i(3,t_count),z_dest(3),yaw_i(3,t_count),yaw_dest(3)];
%[XDOT,YDOT,ZDOT]
end
...
Current=[x_i(1,t_count),y_i(1,t_count),z_i(1,t_count)]
%[volt_1,volt_2]
%z_i(1,t_count)
%[x_i(2,t_count), y_i(2,t_count), z_i(2,t_count)]
coverror(t_count)
time_save(t_count)=toc;
drawnow
...
fprintf('Landing!\n')
fprintf('....Thanks, Michael\n')

while z_1>-100 && z_2>-100 && z_3>-100
for kk=1:k
ctrl_msg(kk).Type = 2;
ctrl_msg(kk).X = 0;
ctrl_msg(kk).Y = 0;
