...
gps_list = {'/gps_1' '/gps_2' '/gps_3' '/gps_4' '/gps_5' '/gps_6'};
imuCallback_list = {@imuCallback_1 @imuCallback_2};
xyzCallback_list = {@xyzCallback_1 @xyzCallback_2 @xyzCallback_3 @xyzCallback_4 @xyzCallback_5 @xyzCallback_6};
...
for i=1:k
    ...
    xyz_sub(i) = rossubscriber(strcat(strcat('/quad',num2str(i)),'/fcu/gps'), 'sensor_msgs/NavSatFix',xyzCallback_list{i});
    pause(1);
    ...
    pos_pub(i) = rospublisher(sprintf('/cov_pos_%d',i),'geometry_msgs/PoseWithCovarianceStamped');
    pos_msg(i) = rosmessage(pos_pub(i));
    pause(1);
    
    imu_sub(i) = rossubscriber(strcat(strcat('/quad',num2str(i)),'/fcu/imu'),'sensor_msgs/Imu',imuCallback_list{i});
end
  ...
  tic;
    ...
    for kk=1:k
        ...
        if kk == 1
            ...
            if accel_cov_1(1) == 0
                accel_cov_1 = [1 0 0 0 1 0 0 0 1]*0.0069;
            end
            if angular_cov_1(1) == 0
                angular_cov_1 = [1 0 0 0 1 0 0 0 1]*1.69e-4;
            end
            if orient_cov_1(1) == 0
                orient_cov_1 = [1 0 0 0 1 0 0 0 1]*1.69e-4;
            end
            
            eul(kk,:) = quat2eul([qw_1,qx_1,qy_1,qz_1]);
            %Maybe we should input the previous euler angles that were
            %filterd as well and use those for the rot operations at the
            %beginning of the UKF code.
            [x_i(kk,t_count),y_i(kk,t_count),z_i(kk,t_count),...
                roll_i(kk,t_count),pitch_i(kk,t_count),yaw_i(kk,t_count)] =...
                UKFnew(x_1,y_1,z_1,...
                eul(kk,3),eul(kk,2),eul(kk,1),...
                ax_1,ay_1,az_1,wx_1,wy_1,wz_1,dt,UKFfirst(kk),...
                pos_cov_1,accel_cov_1,angular_cov_1,orient_cov_1,kk);
            
            
        elseif kk == 2
            
            if accel_cov_2(1) == 0
                accel_cov_2 = [1 0 0 0 1 0 0 0 1]*0.0069;
            end
            if angular_cov_2(1) == 0
                angular_cov_2 = [1 0 0 0 1 0 0 0 1]*1.69e-4;
            end
            if orient_cov_2(1) == 0
                orient_cov_2 = [1 0 0 0 1 0 0 0 1]*1.69e-4;
            end
            
            eul(kk,:) = quat2eul([qw_2,qx_2,qy_2,qz_2]);
            
            [x_i(kk,t_count),y_i(kk,t_count),z_i(kk,t_count),...
                roll_i(kk,t_count),pitch_i(kk,t_count),yaw_i(kk,t_count)] =...
                UKFnew(x_2,y_2,z_2,...
                eul(kk,3),eul(kk,2),eul(kk,1),...
                ax_2,ay_2,az_2,wx_2,wy_2,wz_2,dt,UKFfirst(kk),...
                pos_cov_2,accel_cov_2,angular_cov_2,orient_cov_2,kk);
            
        end
        
        %Send UKF stuff to the vehicle
        
pos_msg(kk).Pose.Pose.Position.X = x_i(kk,t_count);
pos_msg(kk).Pose.Pose.Position.Y = y_i(kk,t_count);
pos_msg(kk).Pose.Pose.Position.Z = z_i(kk,t_count);
q_quat=eul2quat([yaw_i(kk,t_count),pitch_i(kk,t_count),roll_i(kk,t_count)]);
pos_msg(kk).Pose.Pose.Orientation.X=q_quat(2);
pos_msg(kk).Pose.Pose.Orientation.Y=q_quat(3);
pos_msg(kk).Pose.Pose.Orientation.Z=q_quat(4);
pos_msg(kk).Pose.Pose.Orientation.W=q_quat(1);
send(pos_pub(kk),pos_msg(kk));  
        
        
            
%             eul(kk,:) = quat2eul([eval(strcat('qw_',num2str(kk))), eval(strcat('qx_',num2str(kk))), eval(strcat('qy_',num2str(kk))), eval(strcat('qz_',num2str(kk)))]);
%             
%             [x_i(kk,t_count),y_i(kk,t_count),z_i(kk,t_count),...
%                 roll_i(kk,t_count),pitch_i(kk,t_count),yaw_i(kk,t_count)] =...
%                 UKF(eval(strcat('x_',num2str(kk))),...
%                 eval(strcat('y_',num2str(kk))),...
%                 eval(strcat('z_',num2str(kk))),...
%                 eul(kk,3),eul(kk,2),eul(kk,1),...
%                 eval(strcat('ax_',num2str(kk))),...
%                 eval(strcat('ay_',num2str(kk))),...
%                 eval(strcat('az_',num2str(kk))),...
%                 eval(strcat('wx_',num2str(kk))),...
%                 eval(strcat('wy_',num2str(kk))),...
%                 eval(strcat('wz_',num2str(kk))),dt,UKFfirst(kk),...
%                 eval(strcat('pos_cov_',num2str(kk))),...
%                 eval(strcat('accel_cov_',num2str(kk))),...
%                 eval(strcat('angular_cov_',num2str(kk))),...
%                 eval(strcat('orient_cov_',num2str(kk))),kk);
            
            eul(kk,1) = roll_i(kk,t_count);
            eul(kk,2) = pitch_i(kk,t_count);
            eul(kk,3) = yaw_i(kk,t_count);
            
            
        
                x_i(kk,t_count) = x_i(kk,t_count)*(200/25)+100; % convert to MATLAB coordinates
                y_i(kk,t_count) = y_i(kk,t_count)*(200/25)+100;
                z_i(kk,t_count) = z_i(kk,t_count)-100;
        
                %DELETE THE NEXT THREE LINES LATER
        x_i_gpswalk(kk,t_count)=x_i(kk,t_count);
        y_i_gpswalk(kk,t_count)=y_i(kk,t_count);
        z_i_gpswalk(kk,t_count)=z_i(kk,t_count);
...
%             x_i(kk,t_count) = eval(strcat('x_',num2str(kk)));
%             y_i(kk,t_count) = eval(strcat('y_',num2str(kk)));
%             z_i(kk,t_count) = eval(strcat('z_',num2str(kk)));
...
if  t_count>101
if mean(e_hat_save(kk,t_count-100:t_count))<Ep_star && Global_Flag(kk)==0 && 1==0
...
Current=[x_i(2,t_count),y_i(2,t_count),z_i(2,t_count)]
lat_save(t_count)=lat_2;
long_save(t_count)=long_2;
alt_save(t_count)=alt_2;
%[volt_1,volt_2]
%ui
dt=toc;
drawnow
